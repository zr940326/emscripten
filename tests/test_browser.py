...
window.close = function() {
  ...
  def test_modularize(self):
    for opts in [[], ['-O1'], ['-O2', '-profiling'], ['-O2'], ['-O2', '--closure', '1'], ['-s', 'WASM=0']]:
      for args, code in [
        ([], 'Module();'), # defaults
        # use EXPORT_NAME
        ...
        Popen([PYTHON, EMCC, 'test.c', '-s', 'MODULARIZE=1', '-s', 'WASM_ASYNC_COMPILATION=0', '-s', 'SINGLE_FILE=1'] + args + opts).communicate()
        open('a.html', 'w').write('''
...
'''
    ...
    for opts, expect in [
      ...
      (['-s', 'WASM_ASYNC_COMPILATION=1', '-s', 'BINARYEN_METHOD="native-wasm,asmjs"'], 0), # try to force it on, but have it disabled
    ]:
      print(opts, expect)
